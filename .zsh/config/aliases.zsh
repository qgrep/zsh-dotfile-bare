...
alias exa="exa --tree --long --octal-permissions -h -H --level 1"
alias exa -all="exa --tree --level=1 --long  --octal-permissions -h"
alias -g Z='| fzf'

alias -g tw='the-way'alias lln="ls -lhtr  --time-style long-iso | tac | cat -n | tac | sed -s 's/^\s*\([0-9]*\)\s*\(.*\)/[\1]  \2 [\1]/'g && pwd"
...
alias deploy="bash -c $DOTFILES/deploy.sh"
# source ~/.zshrc
alias sc="source $HOME/.zshrc"


# zsh dot path
alias -g zshdir="${ZSHDIR}"
alias -g zshf="${ZDOTDIR}/zfunc"
  alias -g zfunc="${ZDOTDIR}/zfunc"
alias -g zshp="${ZDOTDIR}/plugins"
alias -g zshc="${ZDOTDIR}/config"
alias -g zsht="${ZDOTDIR}/tools"
...
alias zsh-alias="code ~/.zsh/aliases.zsh"
